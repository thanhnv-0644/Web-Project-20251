        ...
        orderItems.map((orderItem) => (
          <div key={orderItem.id} className="order-item-details">
            <div className="info">
              ...
              <p>
                <strong>Trạng thái:</strong> {orderItem.status}
              </p>
              <p>
                <strong>Ngày đặt:</strong>{' '}
                {new Date(orderItem.createdAt).toLocaleDateString('vi-VN')}
              </p>
            </div>
            <div className="info">
              <h3>Thông Tin Khách Hàng</h3>
              <p>
                <strong>Họ tên:</strong> {orderItem.user.name}
              </p>
              <p>
                <strong>Email:</strong> {orderItem.user.email}
              </p>
              <p>
                <strong>Số điện thoại:</strong> {orderItem.user.phoneNumber}
              </p>
              <p>
                <strong>Vai trò:</strong> {orderItem.user.role}
              </p>

              <div className="info">
                <h3>Địa Chỉ Giao Hàng</h3>
                <p>
                  <strong>Địa chỉ:</strong> {orderItem.user.address?.street}
                </p>
                <p>
                  <strong>Phường:</strong> {orderItem.user.address?.ward}
                </p>
                <p>
                  <strong>Thành phố:</strong> {orderItem.user.address?.city}
                </p>
              </div>
            </div>
            <div>
              ...
              />
              <p>
                <strong>Tên:</strong> {orderItem.product.name}
              </p>
              <p>
                <strong>Mô tả:</strong> {orderItem.product.description}
              </p>
              <p>
                <strong>Giá:</strong>{' '}
                {orderItem.product.price.toLocaleString('vi-VN')} VNĐ
              </p>
            </div>
